...
export default function ConsultantConnectionsPage() {
  ...
  const handleDecline = async () => {
    ...
    setIsProcessing(true);
    try {
      const response = await fetch(`/api/consultant/connections/${selectedConnection.id}/decline`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const pendingConnections = connections.filter(
    (c) => c.status === 'requested' && c.initiatedBy === 'founder'
  );
  ...
  const renderConnection = (connection: Connection, showActions: boolean = false) => (
    <Card key={connection.id} className="mb-4">
          ...
          <div>
            <CardTitle className="text-lg">
              {connection.status === 'active'
                ? connection.founderName || 'Founder'
                : 'Founder (details hidden until connected)'}
            </CardTitle>
            {connection.status === 'active' && connection.founderCompany && (
              <CardDescription className="flex items-center gap-1 mt-1">
                <Building2 className="h-3 w-3" />
                {connection.founderCompany}
              </CardDescription>
            )}
          </div>
      ...
      <CardContent>
        ...
        {connection.status === 'active' && connection.founderEmail && (
          <div className="mt-4">
            <a
              href={`mailto:${connection.founderEmail}`}
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Manage your connections with founders.
        </p>
      </div>
