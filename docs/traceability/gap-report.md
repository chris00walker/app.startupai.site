# Story-Code Gap Report

Generated: 2026-01-23T03:37:04.316Z

## Stories Without Implementations

| Story ID | Title | Category |
|----------|-------|----------|
| US-A01 | Search and Find Users | Admin |
| US-A02 | View User Profile and State | Admin |
| US-A03 | Impersonate User (Read-Only) | Admin |
| US-A04 | Retry Failed Workflow | Admin |
| US-A05 | View System Health Dashboard | Admin |
| US-A06 | Manage Feature Flags | Admin |
| US-A07 | View Audit Logs | Admin |
| US-A08 | Change User Role | Admin |
| US-A09 | Export User Data | Admin |
| US-A10 | Run Data Integrity Check | Admin |
| US-A11 | Admin Login and Role Verification | Admin |
| US-A12 | Manage User Billing Issues | Admin |
| US-AS01 | Update Profile | Account Settings |
| US-AS02 | Change Password | Account Settings |
| US-AS03 | Enable 2FA | Account Settings |
| US-AS04 | View Login History | Account Settings |
| US-AS05 | Manage Connected Devices | Account Settings |
| US-B01 | View Billing History | Billing |
| US-B02 | Download Invoice | Billing |
| US-B03 | Update Payment Method | Billing |
| US-B04 | Handle Payment Failure | Billing |
| US-B05 | Request Refund | Billing |
| US-B06 | Change Plan | Billing |
| US-B07 | View Tax Invoice (VAT/GST) | Billing |
| US-B08 | Apply Promo Code | Billing |
| US-B09 | Switch Billing Cycle | Billing |
| US-B10 | Resume After Payment Recovery | Billing |
| US-CT01 | Complete Consultant Trial Onboarding | Consultant Trial |
| US-CT02 | Explore Portfolio with Mock Clients | Consultant Trial |
| US-CT03 | Attempt Real Client Invite (Upgrade Prompt) | Consultant Trial |
| US-CT04 | View Consultant Trial Limits and Status | Consultant Trial |
| US-CT05 | Upgrade to Consultant Plan | Consultant Trial |
| US-CT06 | Experience Post-Upgrade Orientation | Consultant Trial |
| US-N01 | Receive In-App Notification | Notification |
| US-N02 | Receive Email Notification | Notification |
| US-N03 | Manage Notification Preferences | Notification |
| US-N04 | Escalation Alert (Approval Aging) | Notification |
| US-N05 | Unsubscribe from Emails | Notification |
| US-O01 | Cancel Subscription | Offboarding |
| US-O02 | Complete Exit Survey | Offboarding |
| US-O03 | View Data Retention Notice | Offboarding |
| US-O04 | Reactivate Cancelled Account | Offboarding |
| US-O05 | Win-Back Email Response | Offboarding |
| US-S01 | Contact Support | Support |
| US-S02 | View Help Articles | Support |
| US-S03 | Track Support Ticket | Support |
| US-S04 | Request Data Export (GDPR) | Support |
| US-S05 | Delete Account (GDPR) | Support |

**Total Gaps: 48 / 90**

## Gaps by Category

| Category | Count |
|----------|-------|
| Account Settings | 5 |
| Admin | 12 |
| Billing | 10 |
| Consultant Trial | 6 |
| Notification | 5 |
| Offboarding | 5 |
| Support | 5 |