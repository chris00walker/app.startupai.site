...
describe('Alex System Prompt Configuration', () => {
  describe('Stage Awareness', () => {
    ...
    it('should guide Alex to ask 3-4 questions per stage', () => {
      ...
      expect(FOUNDER_SYSTEM_PROMPT).toMatch(/3[-–]?[45]?\s*(question|exchange)/i);
    });

    it('should tell Alex NOT to say "final question"', () => {
      expect(FOUNDER_SYSTEM_PROMPT).toContain('NEVER say "final question"');
    });

    it('should tell Alex to ask ONE question at a time', () => {
      expect(FOUNDER_SYSTEM_PROMPT).toContain('ONE question at a time');
    });
  });
  ...
  describe('Uncertainty Handling', () => {
    it('should acknowledge "I don\'t know" as valuable', () => {
      ...
      expect(FOUNDER_SYSTEM_PROMPT.toLowerCase()).toContain("i don't know");
    });
    ...
    it('should treat uncertainty as a learning opportunity', () => {
      ...
      const hasUncertaintyGuidance =
        FOUNDER_SYSTEM_PROMPT.includes('uncertainty') ||
        FOUNDER_SYSTEM_PROMPT.includes('hypothesis');
  ...
  describe('Team Context', () => {
    it('should mention the AI leadership team', () => {
      expect(FOUNDER_SYSTEM_PROMPT).toContain('Sage');
      expect(FOUNDER_SYSTEM_PROMPT).toContain('Chief Strategy Officer');
    });

    it('should mention handoff to CrewAI team', () => {
      expect(FOUNDER_SYSTEM_PROMPT).toContain('handed off');
    });
  });

  describe('Response Format', () => {
    it('should require ending with a question', () => {
      expect(FOUNDER_SYSTEM_PROMPT).toContain('MUST end with');
      expect(FOUNDER_SYSTEM_PROMPT).toContain('follow-up question');
    });

    it('should specify Acknowledgment → Insight → Question structure', () => {
      expect(FOUNDER_SYSTEM_PROMPT).toContain('Acknowledgment');
      expect(FOUNDER_SYSTEM_PROMPT).toContain('Insight');
      expect(FOUNDER_SYSTEM_PROMPT).toContain('Next Question');
    });
  });
});
...
describe('Alex Initial Greeting', () => {
  it('should introduce Alex by name', () => {
    expect(FOUNDER_INITIAL_GREETING).toContain('Alex');
  });
  ...
  it('should set time expectations (15-20 minutes)', () => {
    expect(FOUNDER_INITIAL_GREETING).toMatch(/15[-–]?20\s*minutes/);
  });
  ...
  it('should acknowledge that "I don\'t know" is valuable', () => {
    expect(FOUNDER_INITIAL_GREETING.toLowerCase()).toContain("i don't know");
  });

  it('should end with a question about the business idea', () => {
    expect(FOUNDER_INITIAL_GREETING).toContain('What business idea');
    expect(FOUNDER_INITIAL_GREETING).toContain('?');
  });
});
...
describe('Stage System Context Generation', () => {
  ...
  it('should include the objective for the stage', () => {
    const stage1 = getFounderStageConfig(1)!;
    const context = getFounderStageSystemContext(1, {});
...
describe('Alex Client Mode (Consultant Acting for Client)', () => {
  ...
  it('should have a way to generate client-mode system context', () => {
    ...
    // getFounderStageSystemContext should support a 'mode' parameter
    const context = getFounderStageSystemContext(1, {});
...
describe('Alex Conversation Behavior', () => {
  ...
  describe('Stage Transitions', () => {
    it('should not allow Alex to determine stage completion', () => {
      ...
      expect(FOUNDER_SYSTEM_PROMPT).toContain('determined by a quality assessment system');
    });

    it('should tell Alex that stages advance automatically', () => {
      // Alex is told the system handles transitions
      expect(FOUNDER_SYSTEM_PROMPT).toContain('system will automatically');
    });
  });

  describe('Response Quality', () => {
    it('should require evidence over assumptions', () => {
      expect(FOUNDER_SYSTEM_PROMPT).toContain('Evidence beats assumptions');
    });

    it('should celebrate specificity', () => {
      expect(FOUNDER_SYSTEM_PROMPT).toContain('specificity');
    });

    it('should handle red flags appropriately', () => {
      expect(FOUNDER_SYSTEM_PROMPT).toContain('Red Flags');
    });
  });
});
